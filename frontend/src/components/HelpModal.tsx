...
export function HelpButton({ onClick, theme = 'modern' }: { onClick: () => void; theme?: 'modern' | 'classic' }) {
  ...
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1.5 text-xs font-medium text-indigo-400 hover:text-indigo-300 border border-indigo-500/30 rounded-lg px-2.5 py-1.5 hover:bg-indigo-500/10 transition-colors"
    >
      ...
      <>
        ...
        {open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setOpen(false)}>
            <div className="bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-lg max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="bg-gray-800 rounded-2xl shadow-2xl border border-gray-700 w-full max-w-lg max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-6 py-3 border-t border-gray-700 bg-gray-800/50 text-right">
              <button onClick={() => setOpen(false)} className="text-sm font-medium text-indigo-400 hover:text-indigo-300 px-4 py-1.5 rounded-lg hover:bg-indigo-500/10 transition-colors">
                Entendido
              </button>
            </div>
          </div>
