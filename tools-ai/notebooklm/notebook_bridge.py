#!/usr/bin/env python3
"""
Kavana V3 NotebookLM CDP bridge.

Local automation layer for Roo Code <-> NotebookLM. It connects to an already
running Chrome instance on localhost:9222, reuses a permanent NotebookLM notebook,
can add temporary external sources, triggers chat/artifacts, extracts visible text
to Markdown, and supports teardown of temporary sources.
"""
...
DEFAULT_NOTEBOOK_URL = "https://notebooklm.google.com/notebook/a8ef67a8-896b-463e-a522-eaae826b3b79"
ACTION_TIMEOUT_MS = 4000
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DRIVE_SYNC_OUTPUT_DIR = PROJECT_ROOT / "docs" / "ai" / "outputs"
LOCAL_CACHE_OUTPUT_DIR = PROJECT_ROOT / "tools-ai" / "notebooklm" / "outputs"


def default_output_dir() -> Path:
    override = os.environ.get("KAVANA_NOTEBOOKLM_OUTPUT_DIR")
    if override:
        return Path(override).expanduser()

    return DRIVE_SYNC_OUTPUT_DIR


DEFAULT_OUTPUT_DIR = default_output_dir()
DEFAULT_SESSION_FILE = DEFAULT_OUTPUT_DIR / "notebooklm_session_latest.json"
...
def research_directive(research_mode: str) -> str:
    if research_mode == "deep":
        return (
            "Modo de investigación: Deep Research. "
            "Explora varias fuentes y sintetiza un informe profundo; usar solo cuando se solicite explícitamente."
        )

    return (
        "Modo de investigación: Fast Research por defecto. "
        "Prioriza documentación oficial, fuentes técnicas de primer nivel y respuestas concretas. "
        "Evita Deep Research para cuidar la cuota diaria."
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Orquesta NotebookLM V3 desde Chrome CDP: cuaderno permanente, fuentes temporales, chat/artefactos, extraccion y limpieza."
    )
    parser.add_argument("--cdp-endpoint", default=CDP_DEFAULT, help="CDP endpoint de Chrome, por defecto http://localhost:9222")
    parser.add_argument("--notebook-url", default=DEFAULT_NOTEBOOK_URL, help="URL directa del cuaderno permanente de NotebookLM")
    parser.add_argument("--notebook-title", default="Kavana V3", help="Título del cuaderno a buscar si no se pasa --notebook-url")
    parser.add_argument("--add-source-url", action="append", default=[], help="URL externa temporal a añadir al cuaderno. Puede repetirse.")
    parser.add_argument("--source-mode", choices=["website", "pasted"], default="website", help="Modo de ingesta: website o pasted. Por defecto website.")
    parser.add_argument("--research-mode", choices=sorted(RESEARCH_MODES), default="fast", help="Modo de investigación de NotebookLM. fast por defecto para cuidar cuota; usa deep solo para investigación extensa.")
    parser.add_argument("--chat-prompt", help="Prompt para enviar al chat de NotebookLM")
    parser.add_argument(
        "--audio-prompt",
        default=(
            "Genera una guia de audio enfocada en multi-tenancy, RLS con tenant_id, "
            "PgBouncer, HMI offline-first, seguridad industrial y UX de vision de tunel de Kavana V3."
        ),
        help="Instruccion para personalizar la guia de audio de NotebookLM.",
    )
    parser.add_argument(
        "--artifact",
        action="append",
        choices=["chat", "flashcards", "quiz", "audio", "all"],
        help="Artefacto a ejecutar. Puede repetirse. Por defecto all.",
    )
    parser.add_argument(
        "--output-dir",
        default=str(DEFAULT_OUTPUT_DIR),
        help="Directorio de salida Markdown. Por defecto docs/ai/outputs; usa KAVANA_NOTEBOOKLM_OUTPUT_DIR para caché local.",
    )
    parser.add_argument("--session-file", default=str(DEFAULT_SESSION_FILE), help="Manifiesto JSON de sesion")
    parser.add_argument("--navigation-timeout-ms", type=int, default=ACTION_TIMEOUT_MS, help="Timeout estricto para acciones UI, por defecto 4000 ms")
    parser.add_argument("--notebook-wait-ms", type=int, default=3000, help="Pausa tras abrir NotebookLM o el cuaderno")
    parser.add_argument("--artifact-wait-ms", type=int, default=12000, help="Pausa de observacion tras solicitar Flashcards o Quiz")
    parser.add_argument("--audio-wait-ms", type=int, default=15000, help="Pausa de observacion tras solicitar audio")
    parser.add_argument("--source-wait-ms", type=int, default=12000, help="Pausa de observacion tras añadir una fuente temporal")
    parser.add_argument("--clean-temp-sources", action="store_true", help="Limpia fuentes temporales registradas y no extrae nuevos resultados")
    parser.add_argument("--cleanup-urls", action="append", default=[], help="URL temporal concreta a limpiar. Puede repetirse.")
...
def assert_local_cdp(endpoint: str) -> None:
    ...
    if parsed.hostname not in {"localhost", "127.0.0.1", "::1"}:
        raise ValueError("Por seguridad, el CDP endpoint debe ser localhost o 127.0.0.1.")
...
def search_notebook(page: Page, title: str, timeout_ms: int) -> None:
    ...
    try:
        ...
        pass
    print(f"[WARN] No se encontro un campo de busqueda para '{title}'. Se continuara en la pagina actual.")
...
def open_permanent_notebook(page: Page, notebook_url: str | None, notebook_title: str, navigation_timeout_ms: int, notebook_wait_ms: int) -> dict[str, Any]:
    ...
    print("[WARN] --notebook-url no proporcionado. Se usara --notebook-title como fallback.")
...
def extract_markdown(page: Page, notebook_name: str, output_dir: Path, session_state: dict[str, Any], timeout_ms: int) -> tuple[Path, int]:
    ...
    if not captured_text.strip():
        raise RuntimeError("No se pudo extraer texto visible de la pagina de NotebookLM.")
...
> Extraccion local desde una sesion automatizada de NotebookLM. Revisar antes de usar como fuente de verdad.
> Roo Code debe presentar el plan y esperar confirmacion explicita del desarrollador antes de modificar codigo.
